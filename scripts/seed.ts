      ...
      for (let i = 0; i < teamIds.length; i++) {
        ...
        if (sessionJuries.includes(team.juryId)) {
          ...
          marksData.push({
            ...
            juryId: team.juryId,
            session: sessionId,
            innovationScore: isSubmitted ? faker.number.int({ min: 60, max: 100 }) : 0,
            presentationScore: isSubmitted ? faker.number.int({ min: 60, max: 100 }) : 0,
            technicalScore: isSubmitted ? faker.number.int({ min: 60, max: 100 }) : 0,
            impactScore: isSubmitted ? faker.number.int({ min: 60, max: 100 }) : 0,
